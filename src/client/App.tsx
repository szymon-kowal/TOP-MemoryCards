...
import { apiKey } from "./assets/components/apiKey";
...
const App: React.FC = () => {
  ...
  function handleSetScore(idx: number): void {
    if (!cards[idx].wasFlipped) {
  ...
  function handleClickCard(idx: number): void {
    setCards((prevCards) => {
      const copy = JSON.parse(JSON.stringify(prevCards)) as CardObj[];

      const clickedCard = copy.find((card) => card.id === idx);

      if (clickedCard !== undefined && !clickedCard.wasFlipped) {
        // Set the specific card's wasFlipped to true
        clickedCard.wasFlipped = true;
      } else {
        resetCards();
      }
      return copy;
  ...
  function resetCards(): void {
    setScore(0);
    setCards(
      shuffleCards(cards.map((item) => ({ ...item, wasFlipped: false })))
    );
  }

  function shuffleCards(cards: CardObj[]): CardObj[] {
  ...
  return (
    <div className="app-container">
      <div className="header">
        <img src={image} alt="Giphy Attribution Logo" className="logo"></img>
      ...
      <div className="footer">
        <div className="footerText">Made by Szymon</div>
        <a href="https://github.com/szymon-kowal/TOP-MemoryCards">
          <svg
